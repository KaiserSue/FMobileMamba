...
class KernelSpatialModulation_Global(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16, 
        ...
        # self.kn = nn.Parameter(to
        # rch.FloatTensor([kernel_num]), requires_grad=True)
...
class FrequencyBandModulation(nn.Module):
    def __init__(self, 
        ...
        #  register_buffer: 在内存中，注册一个永久性Tensor
    ...
    def _precompute_masks(self, max_size, k_list):
        ...
        _, freq_indices = get_fft2freq(d1=max_h, d2=max_w, use_rfft=True) # output: tensor of tuple (fh,fw)
        ...
        for freq in k_list:
            # 创建一个布尔掩码
            mask = freq_indices < 0.5 / freq + 1e-8 # 阈值为0.5/freq 可以实现动态去把跟卷积强相关的频率筛选出来
            # print(freq)
            # print(mask)
            masks.append(mask)
