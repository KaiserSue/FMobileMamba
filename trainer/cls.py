...
class CLSTrainer(BaseTrainer):
	...
	def get_val_loss(self):
		...
		if torch.any(~torch.isfinite(outputs)) or torch.any(~torch.isfinite(self.targets)):
			return 0.0, 0, False
		loss_mean = F.cross_entropy(outputs, self.targets, reduction='mean')
		if not torch.isfinite(loss_mean):
			return 0.0, 0, False
		loss_sum = reduce_tensor(loss_mean * self.bs, self.world_size, sum_avg=False)
		sample_cnt = reduce_tensor(torch.tensor(self.bs).float(), self.world_size, sum_avg=False)
		return loss_sum.clone().detach().item(), int(sample_cnt.clone().detach().item()), True
	
	@torch.no_grad()
	def test(self):
		top1, top5, avg_loss, loss_valid = self.test_net(self.net, name='net', collect_loss=True)
		self.is_best = True if len(self.topk_recorder['net_top1']) == 0 or top1 > max(self.topk_recorder['net_top1']) else False
		self.topk_recorder['net_top1'].append(top1)
		self.topk_recorder['net_top5'].append(top5)
		...
		if self.master and self.loss_recorder:
			if self.epoch >= self.train_loss_cutoff:
				self.loss_recorder.truncate_after(self.train_loss_cutoff)
				log_msg(self.logger, 'skip writing train loss during test stage')
			elif loss_valid and avg_loss is not None:
				self.loss_recorder.record_train_loss(epoch=self.epoch, loss=avg_loss, cutoff_epoch=self.train_loss_cutoff)
			else:
				log_msg(self.logger, 'skip writing train loss due to invalid value')
		if self.ema:
			top1_ema, top5_ema, _, _ = self.test_net(self.net_E, name='net_E')
			self.is_best_ema = True if len(self.topk_recorder['net_E_top1']) == 0 or top1_ema > max(
				self.topk_recorder['net_E_top1']) else False
			self.topk_recorder['net_E_top1'].append(top1_ema)
			self.topk_recorder['net_E_top5'].append(top5_ema)
