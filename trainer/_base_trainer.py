...
class BaseTrainer():
    def __init__(self, cfg):
        ...
        self.topk5 = True if cfg.data.nb_classes > 5 else False
        ...
        cfg.data.train_length, cfg.data.test_length = len(self.train_loader.dataset), len(self.test_loader.dataset)
        # Mixup or Cutmix
        ...
        self.loss_recorder = LossRecorder(cfg.logdir_train, logger=self.logger) if self.master else None
        self.train_loss_cutoff = cfg.trainer.test_start_epoch
        if not hasattr(cfg, 'plot') or cfg.plot is None:
            cfg.plot = Namespace()
        if not hasattr(cfg.plot, 'loss_dir') or cfg.plot.loss_dir is None:
            cfg.plot.loss_dir = cfg.logdir_train
        if not hasattr(cfg.plot, 'loss_filename') or not cfg.plot.loss_filename:
            cfg.plot.loss_filename = 'train_loss.jpg'
        self.loss_output_dir = cfg.plot.loss_dir if cfg.plot.loss_dir else cfg.logdir_train
        self.loss_txt_path = os.path.join(self.loss_output_dir, 'train_loss.txt')
        if cfg.ft:
    ...
    def reset(self, isTrain=True, train_mode=True, prefix=None):
        ...
        self.log_terms, self.progress = get_log_terms(
            ...
            default_prefix=(prefix if prefix else ('Train' if isTrain else 'Test')))
    ...
    def post_update(self):
        if not self.cfg.trainer.mixup_kwargs or not self.isTrain:
            if self.topk5:
                ...
                top15, top15bs_cnt = accuracy(self.outputs['out'], self.targets, topk=(1, 1))
            # top1, top5 = top15
    ...
    def _finish(self):
        ...
        if self.master and self.loss_recorder:
            default_loss_dir = self.cfg.plot.loss_dir if getattr(self.cfg.plot, 'loss_dir', None) else self.cfg.logdir_train
            loss_dir = getattr(self, 'loss_output_dir', default_loss_dir)
            loss_filename = self.cfg.plot.loss_filename if self.cfg.plot.loss_filename else 'train_loss.jpg'
            output_path = self.loss_recorder.plot(
                loss_dir, loss_filename, title='Train Loss', max_epoch=self.train_loss_cutoff)
            log_msg(self.logger, f'loss curve saved to {output_path}') if output_path else None
            self.loss_recorder.export_txt(self.loss_txt_path, max_epoch=self.train_loss_cutoff)
    ...
    @torch.no_grad()
    def test_net(self, net, name='', vis=True, collect_loss=False):
        if collect_loss and not hasattr(self, 'get_val_loss'):
            raise AttributeError('collect_loss=True requires get_val_loss')
        ...
        while batch_idx < test_length:
            ...
            self.post_update()
            if collect_loss:
                batch_loss, batch_sample_cnt, batch_valid = self.get_val_loss()
                loss_valid = loss_valid and batch_valid
                if batch_valid and batch_sample_cnt > 0:
                    total_loss_sum += batch_loss
                    total_sample_cnt += batch_sample_cnt
            t2 = get_timepc()
        ...
        top5 = self.log_terms.get('top5_cnt').sum * 100. / min(self.cfg.data.test_length,
            ...
            'top_all').sum > 0 else 0
        avg_loss = None
        if collect_loss and loss_valid and total_sample_cnt > 0:
            avg_loss = total_loss_sum / total_sample_cnt
        log_msg(self.logger, f'{name}: {top1:.3f} ({top5:.3f})') if vis else None
        return top1, top5, avg_loss, loss_valid
    ...
    def save_checkpoint(self):
        if self.master:
            ...
            if self.is_best:
                best_prev = glob.glob(f'{self.cfg.logdir}/best_epoch*.pth')
                for p in best_prev:
                    os.remove(p)
                best_path = f'{self.cfg.logdir}/best_epoch{self.epoch:03d}.pth'
